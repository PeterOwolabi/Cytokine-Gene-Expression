library(readxl)
TGF_B1_MREC <- read_excel("C:/Users/PETER/Desktop/Work/TGF_B1 MREC.xlsx")
View(TGF_B1_MREC)
library(dplyr)
data <- TGF_B1_MREC |>
...
plot <- ggplot(group_summary, aes(x = Sex_Diet, y = mean_RQ, fill = Sex_Diet)) +
  ...
  labs(x = "Sex_Diet", y = "Relative Expression(2^-DDCt)", title = "TGF_B1 Gene Expression by Interaction of Sex and Diet") +
  theme_classic() +
...
plot <- ggplot(group_summary, aes(x = Breed_Diet, y = mean_RQ, fill = Breed_Diet)) +
  ...
  geom_errorbar(aes(ymin = mean_RQ - se_RQ, ymax = mean_RQ +se_RQ), width = 0.2) +
  geom_text(aes(label = Letters, y = mean_RQ + se_RQ + 0.1), size = 6) +
  labs(x = "Breed_Diet", y = "Relative Expression(2^-DDCt)", title = "TGF_B1 Gene Expression by Interaction of Breed and Diet") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme(text = element_text(size = 14), legend.position = "none")
plot
...
plot <- ggplot(group_summary, aes(x = Breed_Sex, y = mean_RQ, fill = Breed_Sex)) +
  geom_bar(stat = "identity", color = "black", width = 0.4) +
  geom_errorbar(aes(ymin = mean_RQ - se_RQ, ymax = mean_RQ +se_RQ), width = 0.2) +
  labs(x = "Breed_Sex", y = "Relative Expression(2^-DDCt)", title = "TGF_B1_MREC Gene Expression by Interaction of Breed and Sex") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme(text = element_text(size = 14), legend.position = "none")
plot
...
library(multcompView)
letters <- multcompLetters4(anova_result, tukey)
letters_df <- data.frame(Breed_Sex = names(letters$Breed_Sex_Diet$Letters), Letters = letters$Breed_Sex_Diet$Letters)
group_summary <- group_summary |>
...
plot <- ggplot(group_summary, aes(x = Breed_Sex_Diet, y = mean_RQ, fill = Breed_Sex_Diet)) +
  geom_bar(stat = "identity", color = "black", width = 0.4) +
  geom_errorbar(aes(ymin = mean_RQ - se_RQ, ymax = mean_RQ +se_RQ), width = 0.2) +
  labs(x = "Breed_Sex_Diet", y = "Relative Expression(2^-DDCt)", title = "TGF_B1_MREC Gene Expression by Interaction of Breed and Sex") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme(text = element_text(size = 14), legend.position = "none")
plot
...
plot <- ggplot(group_summary, aes(x = Breed, y = mean_RQ, fill = Breed)) +
  geom_bar(stat = "identity", color = "black", width = 0.4) +
  geom_errorbar(aes(ymin = mean_RQ - se_RQ, ymax = mean_RQ +se_RQ), width = 0.2) +
  labs(x = "Breed", y = "Relative Expression(2^-DDCt)", title = "TGF_B1 Gene Expression by Breed") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme(text = element_text(size = 14), legend.position = "none")
plot 
...
plot <- ggplot(group_summary, aes(x = Sex, y = mean_RQ, fill = Sex)) +
  geom_bar(stat = "identity", color = "black", width = 0.4) +
  geom_errorbar(aes(ymin = mean_RQ - se_RQ, ymax = mean_RQ +se_RQ), width = 0.2) +
  labs(x = "Sex", y = "Relative Expression(2^-DDCt)", title = "TGF_B1 Gene Expression by Sex") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme(text = element_text(size = 14), legend.position = "none")
plot
...
plot <- ggplot(group_summary, aes(x = Diet, y = mean_RQ, fill = Diet)) +
  ...
  geom_errorbar(aes(ymin = mean_RQ - se_RQ, ymax = mean_RQ +se_RQ), width = 0.2) +
  geom_text(aes(label = Letters, y = mean_RQ + se_RQ + 0.1), size = 6) +
  labs(x = "Diet", y = "Relative Expression(2^-DDCt)", title = "TGF_B1 Gene Expression by Diet") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme(text = element_text(size = 14), legend.position = "none")
plot
